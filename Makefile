...
generate: $(DEEPCOPY_TARGET) manifests
	...
	cd config/crs && $(GOBIN)/kustomize edit set image exporter=$(REPO_EXPORTER):$(TAG) ruler=$(REPO_RULER):$(TAG_RULER)
...
image-push: image
	...
	docker push $(REPO_RULER):$(TAG_RULER)
...
ruler-image: cmd/ruler/Dockerfile
	docker build -t $(REPO_RULER):$(TAG_RULER) -f cmd/ruler/Dockerfile .
